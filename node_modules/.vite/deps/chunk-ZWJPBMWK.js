...
async function download(options) {
  ...
  if (options.uri.startsWith("ar://")) {
    const { resolveArweaveScheme } = await import("./arweave-JRJPNQVR.js");
...
async function resolvePluginPatternAddresses(contract) {
  try {
    const { readContract: readContract2 } = await import("./read-contract-GBAWQGBB.js");
...
async function estimateGas(options) {
  ...
  const promise = (async () => {
    ...
    const { encode: encode2 } = await import("./encode-REO5UR3T.js");
    ...
    const [{ getRpcClient: getRpcClient2 }, { eth_estimateGas }] = await Promise.all([
      import("./rpc-DT2O3CFY.js"),
      import("./eth_estimateGas-KLYJNHT5.js")
    ]);
...
async function toSerializableTransaction(options) {
  ...
  let [data, nonce, gas, feeData, to, accessList, value] = await Promise.all([
    ...
    (async () => {
      ...
      return from ? await import("./eth_getTransactionCount-ZX2RXR5T.js").then(({ eth_getTransactionCount }) => eth_getTransactionCount(rpcRequest, {
        address: from,
        blockTag: "pending"
      })) : void 0;
    })(),
...
//# sourceMappingURL=chunk-ZWJPBMWK.js.map
